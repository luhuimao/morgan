...
A client *app* interacts with a Morgan cluster by sending it *transactions*
with one or more *instructions*. The Morgan *runtime* passes those instructions
...
ELF shared object containing BPF bytecode and sends it to the Morgan cluster.
...
Berkley Packet Filter (BPF) safe execution environment. The Morgan SDK offers
...
In the same way that a Linux user uses a path to look up a file, a Morgan
